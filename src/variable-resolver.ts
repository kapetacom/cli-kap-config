...
import { explodeEnvVars, toEnvVarName, interpolateDotEnv } from '@kapeta/config-mapper';
...
export const KAPETA_DOTENV = 'kapeta.config.env';
...
class KapetaVariableResolver {
    ...
    private async resolveMappings(envVars: AnyMap<string>) {
        const dotEnvPath = Path.join(this.baseDir, KAPETA_DOTENV);
        try {
