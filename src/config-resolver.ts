...
import { ConfigFileTemplates, ConfigFileWriter } from '@kapeta/config-mapper';

export const KAPETA_CONFIG = 'kapeta.config.yml';

export async function readConfigTemplates(baseDir: string = process.cwd()): Promise<ConfigFileTemplates> {
    const configPath = Path.join(baseDir, KAPETA_CONFIG);
    try {
